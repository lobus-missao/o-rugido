...
from o_rugido import scheduler as sched_mod
...
def test_scheduler_disabled_by_default(monkeypatch):
    """Scheduler desativado quando O_RUGIDO_SCHEDULER não está definido."""
    monkeypatch.delenv("O_RUGIDO_SCHEDULER", raising=False)
    assert sched_mod._is_enabled() is False


def test_scheduler_disabled_when_zero(monkeypatch):
    """Scheduler desativado quando O_RUGIDO_SCHEDULER=0."""
    monkeypatch.setenv("O_RUGIDO_SCHEDULER", "0")
    assert sched_mod._is_enabled() is False


def test_scheduler_disabled_when_false_string(monkeypatch):
    """Scheduler desativado quando O_RUGIDO_SCHEDULER=false."""
    monkeypatch.setenv("O_RUGIDO_SCHEDULER", "false")
    assert sched_mod._is_enabled() is False


def test_scheduler_enabled_when_one(monkeypatch):
    """Scheduler ativado quando O_RUGIDO_SCHEDULER=1."""
    monkeypatch.setenv("O_RUGIDO_SCHEDULER", "1")
    assert sched_mod._is_enabled() is True


def test_scheduler_enabled_when_true(monkeypatch):
    """Scheduler ativado quando O_RUGIDO_SCHEDULER=true."""
    monkeypatch.setenv("O_RUGIDO_SCHEDULER", "true")
    assert sched_mod._is_enabled() is True


def test_scheduler_enabled_when_yes(monkeypatch):
    """Scheduler ativado quando O_RUGIDO_SCHEDULER=yes."""
    monkeypatch.setenv("O_RUGIDO_SCHEDULER", "yes")
    assert sched_mod._is_enabled() is True
...
def test_create_scheduler_respects_scope_env(monkeypatch):
    monkeypatch.setenv("O_RUGIDO_DISPATCH_SCOPE", "piaui")
    sched = sched_mod.create_scheduler()
    job_ids = {j.id for j in sched.get_jobs()}
...
def test_start_scheduler_returns_false_when_disabled(monkeypatch):
    """start_scheduler() retorna False quando O_RUGIDO_SCHEDULER=0."""
    monkeypatch.setenv("O_RUGIDO_SCHEDULER", "0")
...
def test_get_status_enabled_false_when_disabled(monkeypatch):
    """get_status() retorna enabled=False quando O_RUGIDO_SCHEDULER não está definido."""
    monkeypatch.setattr(sched_mod, "_scheduler", None)
    monkeypatch.delenv("O_RUGIDO_SCHEDULER", raising=False)
    status = sched_mod.get_status()
